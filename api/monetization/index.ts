...
export async function createPayment(req: Request, res: Response) {
  try {
    ...
    // Allow subscriptions to be purchased multiple times to extend pro expiry.
    // Prevent buying a lifetime/premium package if user already has lifetime (proExpiresAt === null).
    if (packageConfig.type === 'premium') {
      if (currentUser?.isPro && currentUser.proExpiresAt === null) {
        return res.status(409).json({ error: 'User already has a lifetime Pro plan' });
      }
    }
...
async function applyPackageBenefits(userId: string, packageId: string, itemId: string, transactionId: string) {
  ...
  if (packageConfig.type === 'subscription' || packageConfig.type === 'premium') {
    const durationDays = packageConfig.proDuration || 30;
    const [user] = await db.select().from(users).where(eq(users.id, userId)).limit(1);
    let expiresAt: Date | null = null;

    if (packageConfig.type === 'premium') {
      // Permanent: set proExpiresAt to null to indicate lifetime
      expiresAt = null;
    } else {
      // Subscription: extend existing expiry if present, otherwise set new expiry
...
export async function stripeWebhook(req: Request, res: Response) {
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // If no userId provided, try to resolve by customer email
      if (!userId && session.customer_email) {
        const [u] = await db.select().from(users).where(eq(users.email, session.customer_email)).limit(1);
        if (u) userId = u.id;
      }

      // Update purchase status record if exists
      try {
        await db
        ...
        console.warn('Could not update infinityPayPurchases for session', session.id, uerr);
      }

      // If this was a micro purchase (hint/solution), record it in store_purchases
      const metadataItemId = metadata.itemId || '';
      const packageConfig = packageId ? PACKAGES[packageId] : undefined;
      ...
      // Grant benefits: if subscription/premium, set user.isPro; if coins/micro handle above
      if (packageConfig && (packageConfig.type === 'subscription' || packageConfig.type === 'premium')) {
        try {
          const durationDays = packageConfig.proDuration || (metadata.plan === 'annual' ? 365 : 30);
          ...
          if (userId) {
            ...
            console.warn('Could not find user to grant pro for session', session.id, 'email', session.customer_email);
          }
        } catch (err) {
          console.error('Failed to grant Pro status after webhook:', err);
        }
      } else if (packageConfig && packageConfig.type === 'coins') {
        // Add coins
        const [user] = await db
          .select()
          .from(users)
